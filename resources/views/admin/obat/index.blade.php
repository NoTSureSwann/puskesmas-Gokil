...
@section('title', 'Kelola Obat - SI Puskesmas & Klinik')

@section('content')
<div class="animated-fade">
    <!-- Admin Header -->
    <div class="mb-4">
        <h2 class="fw-bold mb-1">Manajemen Inventaris Obat</h2>
        <p class="text-muted mb-0">Kelola persediaan obat, kategori, satuan, harga, dan batas minimum stok apotek.</p>
    </div>
    ...
    <div class="card card-premium shadow-sm mb-4 p-3">
        <div class="nav nav-pills card-header-pills flex-column flex-md-row gap-2">
            ...
            <a class="nav-link active bg-primary text-white fw-semibold" href="{{ route('admin.obat.index') }}">
    ...
    <div class="row g-4" x-data="{ editingObat: null }">
        ...
        <div class="col-lg-4">
            ...
            <div class="card card-premium shadow-sm p-4 mb-4" x-show="!editingObat">
                <h5 class="fw-bold mb-3 border-bottom pb-2">Tambah Obat</h5>
            ...
            <div class="card card-premium shadow-sm p-4 border border-primary mb-4" x-show="editingObat" x-cloak>
                <div class="d-flex justify-content-between align-items-center mb-3 border-bottom pb-2">
                    <h5 class="fw-bold mb-0 text-primary">Edit Obat</h5>
                    <button type="button" class="btn-close" @click="editingObat = null"></button>
                </div>
                <form :action="'/admin/obat/' + (editingObat ? editingObat.id : '')" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="row g-2">
                        <div class="col-6 mb-3">
                            <label for="edit_stok" class="form-label fw-bold">Stok</label>
                    ...
                    <div class="mb-3">
                        <label for="edit_harga_satuan" class="form-label fw-bold">Harga Satuan (Rp)</label>
                        <input type="number" step="0.01" name="harga_satuan" id="edit_harga_satuan" class="form-control" x-model="editingObat ? editingObat.harga_satuan : 0" required min="0">
                    </div>
        ...
        <div class="col-lg-8">
            <div class="card card-premium shadow-sm p-4">
                <h5 class="fw-bold mb-3 border-bottom pb-2">Daftar Inventaris</h5>
                ...
                <div class="table-responsive">
                    <table class="table table-hover align-middle">
                        ...
                        <tbody>
                            @forelse ($obats as $obat)
                                ...
                                <tr class="{{ $stokRendah ? 'table-danger-subtle' : '' }}">
                                    ...
                                    <td>
                                        ...
                                        <span class="small text-muted">{{ $obat->deskripsi ?: '-' }}</span>
                                    </td>
                                    ...
                                    <td>
                                        <div class="small text-dark">{{ $obat->kategori }}</div>
                                        <span class="small text-muted">Per {{ $obat->satuan }}</span>
                                    </td>
                                    <td>Rp {{ number_format((float) $obat->harga_satuan, 2, ',', '.') }}</td>
                                    <td>
                                        @if ($obat->is_aktif)
                                    ...
                                    <td class="text-center">
                                        <div class="d-flex justify-content-center gap-2">
                                            ...
                                            <form action="{{ route('admin.obat.toggle', $obat->id) }}" method="POST" class="d-inline">
                                                @csrf
                                                <button type="submit" class="btn btn-sm {{ $obat->is_aktif ? 'btn-outline-warning' : 'btn-outline-success' }} py-1 px-2">
                                                    {{ $obat->is_aktif ? 'Nonaktifkan' : 'Aktifkan' }}
                                                </button>
                                            </form>

                                            <form action="{{ route('admin.obat.destroy', $obat->id) }}" method="POST" class="d-inline" onsubmit="return confirm('Apakah Anda yakin ingin menghapus obat ini?');">
                                                @csrf
                                                @method('DELETE')
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="text-center text-muted py-4">Belum ada obat terdaftar.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
